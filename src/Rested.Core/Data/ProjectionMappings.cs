...
namespace Rested.Core.Data
{
    public static class ProjectionMappings
    {
        ...
        internal static IEnumerable<ProjectionMapping> GetProjectionMappings<TProjection>() =>
            GetProjectionMappingsForType(typeof(TProjection));

        internal static IEnumerable<ProjectionMapping> GetProjectionMappingsForType(Type type)
        {
            if (!_registeredMappings.ContainsKey(type))
                throw new ProjectionMappingNotRegisteredException(type);

            var registeredProjectionMappings = _registeredMappings[type];

            var returnedProjectionMappings = new List<ProjectionMapping>();

            foreach (var projectionMapping in registeredProjectionMappings)
            {
